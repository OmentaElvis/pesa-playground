...
<Sheet bind:open>
	<SheetContent class="mt-[36px] w-full max-w-[800px] overflow-y-auto p-8 pb-[92px]">
		<SheetHeader>
			<SheetTitle class="flex flex-col gap-2 text-left">
			...
			<div class="space-y-3">
				<div class="flex flex-col gap-2 text-sm text-muted-foreground">
					<span class="flex items-center gap-2 font-bold">
						<Calendar class="h-4 w-4" /> Timestamp:
					</span>
					<span>{formatTimestamp(log.created_at)}</span>
				</div>
				<div class="flex flex-col gap-2 text-sm text-muted-foreground">
					<span class="flex items-center gap-2 font-bold">
						<Database class="h-4 w-4" />Request ID:
					</span>
					<span>{log.id}</span>
				</div>
				<div class="flex items-center gap-2 text-sm text-muted-foreground">
					<span class="flex items-center gap-2 font-bold">
						<Globe class="h-4 w-4" />Project ID:
					</span>
					<span>{log.project_id}</span>
				</div>
				{#if log.error_desc}
					<div class="flex flex-col gap-2 text-sm text-muted-foreground">
						<span class="flex items-center gap-2 font-bold"><Info class="h-4 w-4" />Help:</span>
						<span>
			...
			<div class="space-y-4">
						...
						<div class="rounded-lg bg-muted/50 p-3">
							{#each Object.entries(parsedRequest.headers) as [key, value]}
								<div class="flex flex-wrap gap-1 py-1 text-sm">
									<code class="rounded bg-background px-2 py-0.5 font-mono text-xs">{key}:</code>
				...
				{#if parsedRequest?.body}
					<div class="space-y-2">
						...
						<pre
							class="max-h-64 overflow-auto rounded-lg bg-muted/50 p-3 font-mono text-xs whitespace-pre-wrap">{formatBody(
								parsedRequest.body
							)}</pre>
					</div>
				{:else}
