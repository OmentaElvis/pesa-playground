<script lang="ts">
    ...
    import { type TransactionCost, type TransactionCostData, createTransactionCost, deleteTransactionCost, listTransactionCosts, updateTransactionCost } from "$lib/api";
    ...
    async function loadCosts() {
        ...
        Object.keys(grouped).sort().forEach(type => {
            ...
            grouped[type].sort((a, b) => a.min_amount - b.min_amount).forEach(cost => {
    ...
    <Table.Root>
        ...
        <Table.Body>
            {#each costs as item}
                ...
                {:else}
                    <Table.Row>
                        <Table.Cell></Table.Cell> <!-- Empty cell for alignment under the type heading -->
                        ...
                        <Table.Cell>
                            <Button variant="ghost" size="icon" onclick={() => openEditDialog(item as TransactionCost)}>
                                <Edit />
                            </Button>
                            <Button variant="ghost" size="icon" onclick={() => openDeleteAlert(item as TransactionCost)}>
                                <Trash2 />
                            </Button>
                        </Table.Cell>
                    </Table.Row>
    ...
    <Dialog.Content>
        ...
        <div class="grid gap-4 py-4">
            ...
            <div class="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="fee-percentage" type="number" bind:value={costData.fee_percentage} class="col-span-3" />
            </div>
        </div>
        <Dialog.Footer>
