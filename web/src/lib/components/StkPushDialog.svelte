			...
			<DialogTitle class="flex items-center gap-2">
				<div>
					<div class="flex gap-2">
						...
						STK Push: Pay
						<span class="text-md text-orange-500">{formatAmount(dialogData?.amount)}</span>
					</div>
					<div class="bg-muted px-2">
						<code class="font-mono text-xs font-light">{dialogData?.checkout_id}</code>
					</div>
				</div>
			</DialogTitle>
		</DialogHeader>
				...
				{#each actionOptions as option}
					<div class="flex space-x-2">
						...
						<Label for={option.id} class="flex-1 cursor-pointer">
							<div class="flex items-start gap-3 transition-colors hover:bg-muted/50">
								<div class="mt-0.5">
									<svelte:component this={option.icon} class="h-5 w-5 text-muted-foreground" />
								</div>
								<div class="flex-1">
									<div class="font-medium">{option.label}</div>
									<div class="mt-1 text-sm text-muted-foreground">{option.description}</div>
								</div>
							</div>
						</Label>
					</div>
