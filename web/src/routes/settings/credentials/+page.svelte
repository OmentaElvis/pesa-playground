...
<main class="container mx-auto space-y-6 p-6">
		<div class="flex items-center justify-between">
			<div>
				<h1 class="text-3xl font-bold tracking-tight text-foreground">Generate B2C Security Credential</h1>
				<p class="mt-1 text-muted-foreground">
				  Generate an encrypted security credential from your initiator password. This credential is required for Business-to-Customer (B2C) API calls.
				</p>
			</div>
		</div>
    <Tabs.Root value="generate">
  		...
  		<div>
  		  ...
  		  <Field.Group>
  		    <Field.Field>
                        ...
                        <Field.Description>Enter the password that will be encrypted to generate your B2C security credential.</Field.Description>
            <InputGroup.Root>
              <InputGroup.Input id="password" required type={showPassword ? "text" : "password" } />
          ...
          <Field.Field>
            ...
            <Field.Field>
              ...
              <InputGroup.Root>
                ...
                <InputGroup.Addon align="inline-end">
          		    <Button variant="outline" class="cursor-pointer" onclick={()=> copyToClipboard(generated || '')}>
          		      <Copy />
          		    </Button>
                </InputGroup.Addon>
              </InputGroup.Root>
      ...
      <Tabs.Content value="public_key">
            ...
            <Field.Description>
              This is the public key currently used by the system. Note that this is a sandbox-specific key and is not the same as the public key provided by Safaricom for production environments.
            </Field.Description>
            <InputGroup.Root>
              <InputGroup.Textarea rows={10} value={$settings.encryption_keys?.public_key} readonly />
              <InputGroup.Addon align="inline-end">
        		    <Button variant="outline" class="cursor-pointer" onclick={()=> copyToClipboard($settings.encryption_keys?.public_key || '')}>
        		      <Copy />
        		    </Button>
              </InputGroup.Addon>
            </InputGroup.Root>
