<script lang="ts">
  ...
  import { formatTransactionAmount, formatTransactionDate, TransactionStatus, TransactionType, resolveAccountAndNavigate } from "$lib/api";
          ...
          >
            ...
            <p class="text-sm leading-relaxed">
              ...
              {:else if transaction.transaction_type == TransactionType.Paybill }
                {@const {date, time, amount, cost, new_balance, transactionId} = formatValues(transaction)}
                <b>{transactionId}</b> Confirmed. <b>{amount}</b> paid to Pay Bill <button class="cursor-pointer hover:underline" onclick={() => resolveAccountAndNavigate(transaction.to_id, goto)}><b>{transaction.to_name}</b></button> for account {transaction.from_name || 'N/A'} on {date} at {time}. New M-PESA balance is {new_balance}. Transaction cost, {cost}
              {:else if transaction.transaction_type == TransactionType.BuyGoods}
                {@const {date, time, amount, cost, new_balance, transactionId} = formatValues(transaction)}
                <b>{transactionId}</b> Confirmed. <b>{amount}</b> paid to Buy Goods <button class="cursor-pointer hover:underline" onclick={() => resolveAccountAndNavigate(transaction.to_id, goto)}><b>{transaction.to_name}</b></button> on {date} at {time}. New M-PESA balance is {new_balance}. Transaction cost, {cost}
