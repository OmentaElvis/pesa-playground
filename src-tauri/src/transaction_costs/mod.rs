    ...
    C: ConnectionTrait,
{
    let rule = Entity::find()
        .filter(db::Column::TransactionType.eq(transaction_type.to_string()))
        .filter(db::Column::MinAmount.lte(amount / 100)) // Convert amount to KES for rule matching
        .filter(db::Column::MaxAmount.gte(amount / 100)) // Convert amount to KES for rule matching
        .one(db)
        .await?;
    ...
    if Entity::find().count(db).await? == 0 {
        let default_costs = vec![
            ...
            TransactionCostActiveModel {
                transaction_type: sea_orm::ActiveValue::Set("Withdraw".to_string()),
            ...
            TransactionCostActiveModel {
                transaction_type: sea_orm::ActiveValue::Set("SendMoney".to_string()),
                min_amount: sea_orm::ActiveValue::Set(1),
                max_amount: sea_orm::ActiveValue::Set(100),
                fee_fixed: sea_orm::ActiveValue::Set(Some(0)), // Free
                fee_percentage: sea_orm::ActiveValue::Set(None),
            ...
            TransactionCostActiveModel {
                transaction_type: sea_orm::ActiveValue::Set("Deposit".to_string()),
                min_amount: sea_orm::ActiveValue::Set(1),
                max_amount: sea_orm::ActiveValue::Set(150000),
